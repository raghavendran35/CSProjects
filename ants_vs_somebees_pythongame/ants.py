    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                assert self.ant.can_contain(insect) or Ant.can_contain(insect, self.ant), "Two ants in {0}".format(self)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=0, max_range=1000):
        ...
        for _ in range(min_range):
            my_place = my_place.entrance

        counter = min_range


        while True:
            if my_place == hive or counter > max_range:
                return None
            elif not my_place.bees:
                my_place = my_place.entrance
                counter += 1
            else:
                return random_or_none(my_place.bees)
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        return ThrowerAnt.nearest_bee(self, hive, self.minimum_range, self.maximum_range)
    # END Problem 6
...
class ShortThrower(ThrowerAnt):
    ...
    max_range = 3
    def nearest_bee(self, hive):
        return ThrowerAnt.nearest_bee(self, hive, self.minimum_range, self.max_range)
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        Ant.__init__(self)
        if QueenAnt.one_exists:
            self.impostor = True
        else:
            self.impostor = False
        QueenAnt.one_exists = True
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while my_place:
                if my_place.ant:
                    if not my_place.ant.buffed:
                        my_place.ant.damage *= 2
                        my_place.ant.buffed = True
                    if hasattr(my_place.ant,'ant') and my_place.ant.ant and not my_place.ant.ant.buffed:
                        my_place.ant.ant.damage *= 2
                        my_place.ant.ant.buffed = True
