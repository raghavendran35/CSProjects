...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_eval(first,env).eval_call(rest,env)
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            return Frame.lookup(Parent1, symbol)
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self,operands.map(lambda args: scheme_eval(args,env)),env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Invalid operation")
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        try:
            return scheme_eval(complete_eval(scheme_apply(self, operands, env)), env, True)
        except:
            raise SchemeError
        # END Problem 21
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            eval1 = eval_all(expressions.first.second, env)
            if not eval1:
                return test
            else:
                return eval1
...
def do_define_macro(expressions, env):
    ...
    target = expressions.first
    try: target.second
    except: raise SchemeError("too few operands")

    if target.first is Pair:
        raise SchemeError
    ...
    if scheme_symbolp(target.first):
        ...
        raise SchemeError('non-symbol: {0}'.format(target.first))
    # END Problem 21
